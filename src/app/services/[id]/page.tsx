...
//                 {row.badge ? (
//                   <span
//                     className={`text-[10.5px] font-semibold px-2.5 py-0.5 rounded-full ${row.badge === "green"
...
//                 <div
//                   key={i}
//                   className={`border rounded-xl overflow-hidden transition-all duration-200 ${isOpen ? "border-[#e8d49a] shadow-md shadow-[#c9a94e]/8" : "border-gray-200 hover:border-gray-300"
//                     }`}
//                 >
//                   <button
...
//                     </span>
//                     <div
//                       className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-200 ${isOpen ? "bg-[#faf5e8] text-[#c9a94e] rotate-180" : "bg-gray-100 text-gray-400"
//                         }`}
//                     >
...
//               rel="noreferrer"
//               className={`flex flex-col items-center py-5 px-4 rounded-2xl border transition-all hover:-translate-y-1 group ${c.highlight
...
import { ServiceContent } from "@/data/services-content";  // ← new file
...
export default async function Page({ params }: PageProps) {
  ...
  const service = (ServiceContent as Record<string, any>)[id];
  if (!service) notFound();
  return <ServiceDetailPage service={service} />;
}

export async function generateStaticParams() {
  return Object.keys(ServiceContent).map((id) => ({ id }));
}